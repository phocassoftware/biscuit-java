...
public class PublicKey {
    ...
    public PublicKey(Algorithm algorithm, byte[] data) throws NoSuchAlgorithmException, InvalidKeySpecException {
        ...
        if (algorithm == Algorithm.Ed25519) {
            this.key = Ed25519KeyPair.public_key(data);
        } else if (algorithm == Algorithm.P256) {
            this.key = P256KeyPair.public_key(data);
        } else {
    ...
    static public PublicKey deserialize(Schema.PublicKey pk) throws Error.FormatError.DeserializationError, NoSuchAlgorithmException, InvalidKeySpecException {
        if(!pk.hasAlgorithm() || !pk.hasKey() || (pk.getAlgorithm() != Algorithm.Ed25519 && pk.getAlgorithm() != Algorithm.P256)) {
